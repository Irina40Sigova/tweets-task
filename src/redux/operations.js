...
axios.defaults.baseURL = "https://646b4d3e7d3c1cae4ce39595.mockapi.io";

export const fetchTotalPages = createAsyncThunk(
  "users/fetchTotalPages",
  async (_, thunkAPI) => {
    try {
  ...
  async (currentPage, thunkAPI) => {
    try {
      const response = await axios.get(
        `/users?isFollowed=false&page=${currentPage}&limit=3`
      );
      return response.data;
    } catch (e) {
      return thunkAPI.rejectWithValue(e.message);
...
export const fetchFollowing = createAsyncThunk(
  ...
  async (currentPage, thunkAPI) => {
    try {
      const response = await axios.get(
        `/users?isFollowed=true&page=${currentPage}&limit=3`
      );
      return response.data;
    } catch (e) {
      return thunkAPI.rejectWithValue(e.message);
